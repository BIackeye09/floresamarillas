...
/* ------------------------------------------------------------------
   9. Fotografía
   La ruta está en index.html (assets/foto.jpg). Si el archivo no
   existe, la tarjeta se mantiene con un fondo suave y sin errores.
------------------------------------------------------------------ */
function prepararFoto() {
  const foto = $("#foto");
  foto.addEventListener("error", () => {
    foto.style.display = "none";
    console.info("No se encontró assets/foto.jpg. Coloca ahí la fotografía.");
  });
}


/* ------------------------------------------------------------------
   10. Música (opcional, nunca automática)
------------------------------------------------------------------ */
function prepararMusica() {
  ...
  // Si el archivo no existe o no se puede leer, escondemos el botón
  audio.addEventListener("error", ocultar);
  audio.addEventListener("loadedmetadata", () => { boton.hidden = false; });

  // Lo mostramos igual: hay navegadores que no cargan el audio hasta
  // que la persona lo pide. Si falla al reproducir, se oculta.
  boton.hidden = false;

  const marcar = (sonando) => {
  ...
  boton.addEventListener("click", () => {
    ...
    if (audio.paused) {
      ...
      if (intento && typeof intento.then === "function") {
        intento.then(() => marcar(true)).catch(ocultar);
      } else {
        marcar(true);
      }
    } else {
